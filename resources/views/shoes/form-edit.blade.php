        ...
        <div class="max-w-xl mx-auto bg-white p-10 rounded-3xl shadow-lg">
            ...
            <div class="mt-8">
                <form action="{{ route('shoes.update', $shoes->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid gap-4 lg:gap-6">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                            <div>
                                ...
                                <input type="text" value="{{ $shoes->name }}" name="name" class="py-3 px-4 transition-all duration-300 block w-full border-gray-200 rounded-lg text-sm focus:border-[#1E293B] focus:ring-[#1E293B] disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600">
                            </div>

                            <div>
                                <label for="size" class="block mb-2 text-sm text-gray-700 font-medium dark:text-white">Ukuran</label>
                                <input type="number" value="{{ $shoes->size }}" name="size" class="py-3 px-4 transition-all duration-300 block w-full border-gray-200 rounded-lg text-sm focus:border-[#1E293B] focus:ring-[#1E293B] disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600">
                            </div>
                        </div>

                        <div>
                            <label for="price" class="block mb-2 text-sm text-gray-700 font-medium dark:text-white">Harga</label>
                            <input type="number" value="{{ $shoes->price }}" name="price" class="py-3 px-4 transition-all duration-300 block w-full border-gray-200 rounded-lg text-sm focus:border-[#1E293B] focus:ring-[#1E293B] disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600">
                        </div>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                            <div>
                                <label for="stock" class="block mb-2 text-sm text-gray-700 font-medium dark:text-white">Stok</label>
                                <input type="number" value="{{ $shoes->stock }}" name="stock" class="py-3 px-4 transition-all duration-300 block w-full border-gray-200 rounded-lg text-sm focus:border-[#1E293B] focus:ring-[#1E293B] disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600">
                            </div>
                                ...
                                <div class="relative p-4 w-full max-w-2xl max-h-full">
                                    ...
                                    <div class="relative bg-white rounded-3xl shadow dark:bg-gray-700">
                                        ...
                                        <div class="flex items-center p-4 md:p-5 border-t rounded-b dark:border-gray-600">
                                            <button data-modal-hide="shoes-modal-{{ $shoes->id }}" type="button" class="w-full py-2.5 px-5 text-sm font-medium text-white focus:outline-none bg-[#1E293B] rounded-3xl hover:shadow-md hover:shadow-slate-600 duration-300 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">TUTUP</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="mt-8 grid">
                        <button class="w-full py-3 px-4 transition-all duration-300 inline-flex justify-center items-center gap-x-2 text-sm font-medium border border-transparent bg-[#1E293B] text-white rounded-3xl focus:outline-none hover:shadow-md hover:shadow-slate-600 focus:bg-[#1E293B] disabled:opacity-50 disabled:pointer-events-none">Ubah</button>
                    </div>
                </form>
            </div>
        </div>
